                    ...
                    <div class="form-row align-items-center">
                        <div class="col-sm-4 my-1">
                            <input type="text" class="form-control" name="search"
                                placeholder="Search Functional Location" value="{{ request()->get('search') }}">
                        </div>
                        ...
                        <div class="col-auto my-1">
                            <select class="btn btn-secondary dropdown-toggle" name="filter" onchange="this.form.submit()">
                                <option value="" {{ request()->get('filter') == '' ? 'selected' : '' }}>All</option>
                                <option value="rectified" {{ request()->get('filter') == 'rectified' ? 'selected' : '' }}>Rectified</option>
                                <option value="not_rectified" {{ request()->get('filter') == 'not_rectified' ? 'selected' : '' }}>Not Rectified</option>
                            </select>
                        </div>
                    ...
                    <table class="table table-bordered mt-3">
                        <thead class="thead-dark">
                            <tr>
                                ...
                                <th class="sortable health-status-header" data-column="health_status" data-order="asc">
                                    Criticality</th>
                                <th>Defect</th>
                                <th>TEV</th>
                                <th>Hotspot</th>
                                <th>Acknowledgment Date</th>
                                <th>Start Progres Date</th>
                        ...
                        <tbody id="assetTableBody">
                            @foreach ($assets as $asset)
                                <tr>
                                    ...
                                    <td>
                                        <a href="#" class="popup-link" data-toggle="modal" data-target="#infoModal"
                                            data-info="{{ $asset->Functional_Location }}" 
                                            data-defect="{{ $asset->Defect }}"
                                            data-defect1="{{ $asset->Defect1 }}"
                                            data-defect2="{{ $asset->Defect2 }}"
                                            data-average="{{ $asset->Average }}"
                                            data-pending="{{ $asset->PendingDays }}">
                                    ...
                                    <td>
                                        @php
                                            ...
                                            if (!$asset->completed_status && $asset->Target_Date) {
                                                ...
                                                $pending_days = $targetDate->isPast()
                                                    ? \Carbon\Carbon::now()->diffInDays($targetDate)
                                                    : 0;
                                            } else {
                                                $pending_days = 0;
                                            }
                                    ...
                                    <td>
                                        ...
                                        @else
                                            <form action="{{ route('assets.acknowledge', $asset->id) }}" method="POST">
                                                @csrf
                                                <button type="submit"
                                                    class="btn btn-outline-primary btn-sm">Acknowledge</button>
                                            </form>
                                        @endif
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        @if ($asset->completed_status)
                                            &#10004;
                                        @elseif ($asset->acknowledgment_status)
                                            <button class="btn btn-primary btn-sm update-button" data-toggle="modal"
                                                data-target="#updateModal{{ $asset->id }}">Update</button>
                                        @endif
                                    </td>
                                    <td>
                                        @if ($asset->delete_request)
                                            <button class="btn btn-outline-info btn-sm" disabled>Delete Request
                                                Submitted</button>
                                        @else
                                ...
                                <div class="modal fade" id="updateModal{{ $asset->id }}" tabindex="-1" role="dialog"
                                    aria-labelledby="updateModalLabel{{ $asset->id }}" aria-hidden="true">
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="updateModalLabel{{ $asset->id }}">Update
                                                    Asset</h5>
                                            ...
                                            <form action="{{ route('assets.updateStatus', $asset->id) }}" method="POST">
                                                ...
                                                <div class="modal-body">
                                                    <div class="form-group">
                                                        <label for="rectifierName{{ $asset->id }}">Rectifier's
                                                            Name</label>
                                                        <select class="form-control"
                                                            id="rectifierName{{ $asset->id }}" name="rectifierName"
                                                            required>
                                                    ...
                                                    <div class="form-group">
                                                        ...
                                                        <input type="date" class="form-control"
                                                            id="progressDate{{ $asset->id }}" name="progressDate"
                                                            required>
                                                    </div>
                                                    <div class="form-group">
                                                        <label for="rectifyStatus{{ $asset->id }}">Rectify
                                                            Status</label>
                                                        <select class="form-control"
                                                            id="rectifyStatus{{ $asset->id }}" name="rectifyStatus"
                                                            required>
                                                            <option value="ongoing">Start Progress</option>
                                ...
                                <div class="modal fade" id="deleteModal{{ $asset->id }}" tabindex="-1"
                                    role="dialog" aria-labelledby="deleteModalLabel{{ $asset->id }}"
                                    aria-hidden="true">
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            <form method="POST"
                                                action="{{ route('assets.delete', ['id' => $asset->id]) }}">
                                                @csrf
                                                <div class="modal-header">
                                                    <h5 class="modal-title" id="deleteModalLabel{{ $asset->id }}">
                                                        Delete Asset</h5>
                                                    <button type="button" class="close" data-dismiss="modal"
                                                        aria-label="Close">
                                                        <span aria-hidden="true">&times;</span>
                                                ...
                                                <div class="modal-body">
                                                    <div class="form-group">
                                                        <label for="reason{{ $asset->id }}">Reason for
                                                            deletion:</label>
                    ...
                    <ul class="nav nav-tabs" id="assetTabs" role="tablist">
                        <li class="nav-item">
                            <a class="nav-link active" id="general-tab" data-toggle="tab" href="#general"
                                role="tab" aria-controls="general" aria-selected="true">General Info</a>
                        </li>
                    </ul>
                    <div class="tab-content" id="assetTabsContent">
                        <div class="tab-pane fade show active" id="general" role="tabpanel"
                            ...
                            <form id="infoForm">
                                ...
                                <div class="form-group">
                                    <label for="average">Average Days</label>
                                    <input type="text" class="form-control" id="average" readonly>
                                </div>
                                <div class="form-group">
                                    <label for="pending">Pending Days</label>
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.confirm-delete', function() {
                ...
                $.ajax({
                    url: "{{ route('assets.delete', ['id' => ':asset_id']) }}".replace(':asset_id',
                        assetId),
                    type: 'POST',
                    data: {
            ...
            $('.popup-link').on('click', function() {
                ...
                $('#defect2').val(defect2);
                var average = $(this).data('average') || 0;  
                $('#average').val(average + ' days');

                var pending = $(this).data('pending') || 0;  
                $('#pending').val(pending + ' days');
            });
